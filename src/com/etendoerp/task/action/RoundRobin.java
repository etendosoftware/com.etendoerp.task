...
/**
 * Assigns users to tasks using a Round Robin strategy.
 */
public class RoundRobin extends Action {
  private static final Logger log = LogManager.getLogger(RoundRobin.class);

  /**
   * Assigns a user to tasks, using RoundRobinStrategy.
   *
   * @param parameters
  ...
  protected ActionResult action(JSONObject parameters, MutableBoolean isStopped) {
    ...
    try {
      ...
      RoundRobinHelper.assignUsers(
          taskList,
          taskType -> new RoundRobinStrategy().findUserAccordingStrategy(taskType)
      );

      result.setMessage(OBMessageUtils.messageBD("ETASK_UserAssignedToTask"));
